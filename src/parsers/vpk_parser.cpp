...
namespace unpaker::parsers
{
    ...
    std::string VpkParser::read_cstring(std::ifstream &file)
    {
        ...
        if (result.length() >= MAX_STRING_LEN && ch != '\0')
        {
            std::cerr << "[ERROR] VPK: String exceeded max length (" << MAX_STRING_LEN
                      << " chars) at offset " << (file.tellg() - static_cast<std::streamoff>(1))
                      << std::endl;
            size_t skip_count = 0;
            ...
            while (skip_count < MAX_SKIP)
            {
                if (!file.get(ch))
                {
                    std::cerr << "[ERROR] VPK: End of file while skipping overflow string"
                              << std::endl;
    ...
    bool VpkParser::parse_vpk_v2(std::ifstream &file, std::shared_ptr<DirectoryEntry> &root,
                                 uint32_t &file_count)
    {
        ...
        Logger::instance().info(std::string("VPK: Version=") + std::to_string(version) +
                                std::string(", TreeSize=") + std::to_string(tree_size));
        ...
        if (version == 2)
        {
            ...
            std::cout << "[INFO] VPK: FileDataSectionSize=" << file_data_section_size
                      << ", TreeCRC=0x" << std::hex << tree_crc << ", FileCRC=0x" << file_crc
        ...
        DEBUG_COUT("[DEBUG] VPK: Tree starts at offset "
                   << tree_offset << ", tree_size=" << tree_size << ", tree should end at offset "
                   << tree_end_pos << ", file size=" << file_size << std::endl);

        uint32_t file_count_local = 0;

        auto is_error_marker = [](const std::string &s) {
            return s.find("[OVERFLOW]") != std::string::npos ||
        ...
        while (file.tellg() < tree_end_pos && file.good())
        {
            ...
            {
                std::cerr << "[ERROR] VPK: Found error marker in extension name, stopping parse"
                          << std::endl;
                break;
            }
            ...
            if (ext_name.empty())
            {
                ...
                if (current + static_cast<std::streamoff>(2) > tree_end_pos)
                    break;
            ...
            while (file.tellg() < tree_end_pos && file.good())
            {
                ...
                {
                    std::cerr
                        << "[ERROR] VPK: Found error marker in directory name, aborting extension"
                        << std::endl;
                    break;
                }
                ...
                while (file.tellg() < tree_end_pos && file.good())
                {
                    ...
                    {
                        std::cerr << "[ERROR] VPK: File position did not advance after reading "
                                     "file name at offset "
                    ...
                    if (file.tellg() > tree_end_pos)
                    {
                        std::cerr << "[WARNING] VPK: Read past tree boundary at offset "
                                  << file.tellg() << std::endl;
                        break;
                    }
                    ...
                    {
                        std::cerr
                            << "[ERROR] VPK: Found error marker in file name, aborting directory"
                            << std::endl;
                        break;
                    }
                    ...
                    if (file_name.length() > 0 &&
                        (file_name[0] == '+' || file_name[0] == '{' || file_name[0] == '~'))
                    {
                        DEBUG_CERR("[DEBUG] VPK: Reading suspicious file name \""
                                   << file_name << "\" at offset " << before_read
                                   << ", after read at " << after_read
                                   << ", bytes read: " << (after_read - before_read)
                                   << ", expected: " << (file_name.length() + 1) << std::endl);

                        std::streamoff expected_offset =
                            before_read + static_cast<std::streamoff>(file_name.length() + 1);
                        if (after_read != expected_offset)
                        {
                            std::cerr << "[ERROR] VPK: Position mismatch! Expected "
                                      << expected_offset << " but got " << after_read
                                      << " (difference: " << (after_read - expected_offset)
                                      << " bytes)" << std::endl;
                            std::cerr << "[ERROR] VPK: This indicates the string was not read "
                                         "correctly - "
                    ...
                    if (current_pos + static_cast<std::streamoff>(18) > tree_end_pos)
                    {
                        std::cerr << "[WARNING] VPK: Not enough space for file metadata at "
                                  ...
                                  << (tree_end_pos - current_pos) << "), stopping parse"
                                  << std::endl;
                    ...
                    {
                        std::cerr << "[ERROR] VPK: Failed to read CRC at offset " << file.tellg()
                                  << std::endl;
                        break;
                    }
                    if (!file.read(reinterpret_cast<char *>(&archive_index), 4))
                    {
                        std::cerr << "[ERROR] VPK: Failed to read archive_index at offset "
                                  << file.tellg() << std::endl;
                        break;
                    }
                    if (!file.read(reinterpret_cast<char *>(&entry_offset), 4))
                    {
                        std::cerr << "[ERROR] VPK: Failed to read entry_offset at offset "
                                  << file.tellg() << std::endl;
                        break;
                    }
                    if (!file.read(reinterpret_cast<char *>(&entry_size), 4))
                    {
                        std::cerr << "[ERROR] VPK: Failed to read entry_size at offset "
                                  << file.tellg() << std::endl;
                        break;
                    }
                    ...
                    {
                        std::cerr << "[ERROR] VPK: Failed to read terminator flag at offset "
                                  << file.tellg() << std::endl;
                        break;
                    }
                    ...
                    {
                        std::cerr
                            << "[ERROR] VPK: Metadata read error! Expected 18 bytes, but read "
                            << metadata_bytes_read << " bytes at offset " << before_metadata
                            << std::endl;
                        break;
                    }
                    ...
                    if (term_flag == 0xffff)
                    {
                        ...
                        if (ext_name.length() > 50 || dir_name.length() > 256 ||
                            file_name.length() > 256)
                        {
                            std::cerr
                                << "[WARNING] VPK: Skipping entry with suspiciously long names: "
                                << "ext=" << ext_name.length() << ", dir=" << dir_name.length()
                        ...
                        for (char c : ext_name)
                        {
                            if (static_cast<unsigned char>(c) < 32 || c > 126)
                            {
                                valid = false;
                                break;
                            }
                        ...
                        {
                            std::cerr
                                << "[WARNING] VPK: Skipping entry with invalid characters (file: "
                                << file_name << ")" << std::endl;
                            continue;
                        }
                        ...
                        {
                            entry->path.reserve(dir_name.length() + file_name.length() +
                                                ext_name.length() + 2);
                            entry->path = dir_name;
                            entry->path += '/';
                    ...
                    else
                    {
                        ...
                        std::cerr << "[ERROR] VPK: Invalid terminator 0x" << std::hex << term_flag
                                  << std::dec << " expected 0xffff at offset "
                                  << (err_pos - static_cast<std::streamoff>(2))
        ...
        Logger::instance().info(std::string("VPK: Parsed ") + std::to_string(file_count_local) +
                                std::string(" file entries from v2 archive"));
    ...
    void VpkParser::build_directory_structure(std::shared_ptr<DirectoryEntry> &root)
    {
                ...
                while (pos < dir_path.length())
                {
                    ...
                    if (!dir_name.empty() && dir_name != ".")
                    {
                        ...
                        if (dir_map.find(current_path) == dir_map.end())
                        {
                            ...
                            if (!new_dir)
                            {
                                std::cerr << "[ERROR] VPK: Memory allocation failed for directory"
                                          << std::endl;
                                return;
                            }
        ...
        root->files.erase(std::remove_if(root->files.begin(), root->files.end(),
                                         [&files_to_move](const std::shared_ptr<FileEntry> &f)
                                         {
                                             return std::any_of(files_to_move.begin(),
                                                                files_to_move.end(),
    ...
    bool VpkParser::parse_vpk_dir(std::ifstream &file, std::shared_ptr<DirectoryEntry> &root,
                                  uint32_t &file_count)
    {
        ...
        std::cout << "[INFO] VPK: Dir Version=" << version << ", TreeCRC=0x" << std::hex << tree_crc
                  << std::dec << ", TreeSize=" << tree_size << ", FileCRC=0x" << std::hex
                  << file_crc << std::dec << ", MetaCRC=0x" << std::hex << meta_crc << std::dec
                  << ", ContentCRC=0x" << std::hex << content_crc << std::dec << std::endl;
        ...
        if (tree_size == 0 || tree_size > static_cast<uint32_t>(file_size - tree_offset))
        {
            ...
            {
                std::cout
                    << "[WARNING] VPK: Could not find valid tree start, using default offset 12"
                    << std::endl;
                tree_offset = 12;
            }
        }
        ...
        {
            std::cout << "[INFO] VPK: Using tree_size from header: " << tree_size << " bytes"
                      << std::endl;
        }

        file.seekg(tree_offset);

        DEBUG_COUT("[DEBUG] VPK Dir: Starting tree parsing at offset "
                   << tree_offset << ", file size: " << file_size << std::endl);
        ...
        while (file.tellg() < tree_end_pos && file.good())
        {
            ...
            while (file.tellg() < tree_end_pos && file.good())
            {
                ...
                while (file.tellg() < tree_end_pos && file.good())
                {
                    ...
                    if (term_flag == 0xffff)
                    {
                        if (ext_name.length() > 50 || dir_name.length() > 512 ||
                            file_name.length() > 512)
                        {
                            std::cerr
                                << "[WARNING] VPK: Skipping entry with suspiciously long names: "
                                << "ext=" << ext_name.length() << ", dir=" << dir_name.length()
                        ...
                        if (!valid)
                        {
                            std::cerr << "[WARNING] VPK: Skipping entry with invalid characters"
                                      << std::endl;
                            continue;
                        }
        ...
        std::cout << "[INFO] VPK: Parsed " << file_count_local
                  << " file entries from directory file" << std::endl;
        return file_count_local > 0;
    }

    bool VpkParser::parse(const fs::path &archive_path, std::shared_ptr<DirectoryEntry> &root,
                          uint32_t &file_count)
    {
        std::ifstream file(archive_path, std::ios::binary);
        if (!file)
        ...
        else
        {
            ...
            {
                std::cerr << "[ERROR] VPK: Invalid signature: 0x" << std::hex << signature
                          << std::dec << " in file: " << filename << std::endl;
                std::cerr << "[INFO] Valid VPK files should have signature 0x55aa1234 or 0x465456"
                          << std::endl;
            }
            return false;
        }
                                        ...
                                        std::vector<uint8_t> &data) const
    {
        try
        {
            ...
            if (!ifs.is_open())
            {
                DEBUG_CERR("[DEBUG] VPK: Could not open data file: " << data_file_path.string()
                                                                     << std::endl);
                return false;
            }
            ...
            if (file->offset >= static_cast<uint32_t>(total_size) ||
                static_cast<std::uint64_t>(file->offset) + static_cast<std::uint64_t>(file->size) >
                    static_cast<std::uint64_t>(total_size))
            {
                DEBUG_CERR("[DEBUG] VPK: Data range out of bounds in "
                           << data_file_path.string() << " (offset=" << file->offset << ", size="
                           << file->size << ", total=" << total_size << ")" << std::endl);
                return false;
            }
            ...
            if (!ifs.good())
            {
                DEBUG_CERR("[DEBUG] VPK: Failed to seek to offset "
                           << file->offset << " in " << data_file_path.string() << std::endl);
                return false;
            }
            ...
            if (!ifs.good() && !ifs.eof())
            {
                DEBUG_CERR("[DEBUG] VPK: Read error from data file: " << data_file_path.string()
                                                                      << std::endl);
                return false;
            }
            ...
            if (bytes_read == 0)
            {
                DEBUG_CERR("[DEBUG] VPK: Zero bytes read from data file: "
                           << data_file_path.string() << std::endl);
                return false;
            }

            if (bytes_read != file->size)
            {
                std::cerr << "[WARNING] VPK: Expected to read " << file->size << " bytes, but read "
                          << bytes_read << " bytes from " << data_file_path.string() << std::endl;
        ...
        {
            std::cerr << "[ERROR] VPK: Exception in read_from_data_file(" << data_file_path.string()
                      << "): " << e.what() << std::endl;
            return false;
        }
    }
                                                  ...
                                                  std::vector<uint8_t> &data) const
    {
        try
        {
            ...
            for (const auto &entry : fs::directory_iterator(dir))
            {
                ...
                if (fname.find("_dir.vpk") != std::string::npos)
                    continue;
                if (fname.rfind(prefix, 0) != 0)
                    continue; // must start with prefix

                any_tried = true;
                DEBUG_CERR("[DEBUG] VPK: Fallback trying data archive: " << p.string()
                                                                         << std::endl);
        ...
        {
            std::cerr << "[ERROR] VPK: Exception in fallback_search_data_archives: " << e.what()
                      << std::endl;
            return false;
        }
    }
