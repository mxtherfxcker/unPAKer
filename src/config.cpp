...
namespace unpaker
{
    ...
    Config::Config() : current_theme(ThemeType::SYSTEM), last_file_format(0), dev_mode(false)
    {
        load_from_disk();
    }
    ...
    fs::path Config::get_config_path()
    {
        ...
        if (GetTempPathW(MAX_PATH, temp_path))
        {
            ...
            {
                std::cerr << "[WARNING] Failed to create config directory: " << e.what()
                          << std::endl;
            }
        }
    ...
    COLORREF Config::get_bg_color() const
    {
        ...
        {
        case ThemeType::OLD_LIGHT:
            return RGB(240, 240, 240);
        case ThemeType::OLD_DARK:
            return RGB(30, 30, 30);
        case ThemeType::NEW_LIGHT:
            return RGB(248, 248, 248);
        case ThemeType::NEW_DARK:
            return RGB(25, 25, 25);
        ...
        default:
                              ...
                              0, KEY_READ, &hKey) == ERROR_SUCCESS)
            {
                ...
                LONG ret = RegQueryValueExW(hKey, L"AppsUseLightTheme", nullptr, nullptr,
                                            (LPBYTE)&value, &size);
                RegCloseKey(hKey);
