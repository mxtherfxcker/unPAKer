...
namespace unpaker::parsers
{
    ...
    bool GenericParser::parse(const fs::path &, std::shared_ptr<DirectoryEntry> &, uint32_t &)
    {
                                     ...
                                     std::vector<uint8_t> &data) const
    {
        ...
        try
        {
            ...
            if (!ifs.is_open())
            {
                std::cerr << "[ERROR] Generic: Failed to open archive file: "
                          << archive_path.string() << std::endl;
                return false;
            }
            ...
            if (!ifs.good())
            {
                std::cerr << "[ERROR] Generic: Failed to seek to file offset: " << file->offset
                          << std::endl;
                return false;
            }
