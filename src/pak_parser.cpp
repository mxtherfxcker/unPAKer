...
namespace unpaker
{
        ...
        if (parse_result)
        {
            ...
            if (file_count == 0)
            {
                Logger::instance().warning(
                    "No entries found. This might be a file list or metadata file");
            }
        }
        else
        {
    ...
    bool PakParser::build_directory_tree()
    {
        for (const auto &file : root_directory->files)
        {
            ...
            while ((pos = path.find_first_of("/\\", pos)) != std::string::npos)
            {
                ...
                auto it =
                    std::find_if(current->subdirectories.begin(), current->subdirectories.end(),
    ...
    bool PakParser::extract_file(const std::shared_ptr<FileEntry> &file,
