...
namespace unpaker
{
    ...
    bool GuiManager::initialize(int width, int height, const std::string &title)
    {
        ...
        main_window = CreateWindowExW(WS_EX_LEFT, L"unPAKerWindowClass", wide_title.c_str(),
    ...
    LRESULT GuiManager::handle_message(UINT msg, WPARAM wparam, LPARAM lparam)
    {
        ...
        case WM_COMMAND:
        {
            ...
            { // About
                ...
                MultiByteToWideChar(CP_UTF8, 0, version_str.c_str(), -1, version_wide.data(),
                                    version_len);
        ...
        case WM_NOTIFY:
        {
            ...
            if (hdr->hwndFrom == tree_view && hdr->code == TVN_SELCHANGED)
            {
                ...
                if (item.hItem != NULL)
                {
                    ...
                    int size_needed =
                        WideCharToMultiByte(CP_UTF8, 0, item_text, -1, NULL, 0, NULL, NULL);
                    std::string selected_name(size_needed, 0);
                    WideCharToMultiByte(CP_UTF8, 0, item_text, -1, &selected_name[0], size_needed,
                                        NULL, NULL);
                    ...
                    if (root)
                    {
                        std::shared_ptr<FileEntry> found_file =
                            find_file_in_tree(root, selected_name);
                        if (found_file)
                        {
            ...
            {
                InvalidateRect(tree_view, nullptr, FALSE);
                RedrawWindow(tree_view, nullptr, nullptr,
    ...
    void GuiManager::handle_open_file()
    {
        if (is_loading)
        {
            MessageBoxW(main_window, L"Already loading archive!", L"Information",
                        MB_OK | MB_ICONINFORMATION);
            return;
        }
        ...
        ofn.lpstrFilter =
            L"PAK Files (*.pak)\0*.pak\0VPK Files (*.vpk)\0*.vpk\0All Files (*.*)\0*.*\0";
        ...
        if (GetOpenFileNameW(&ofn))
        {
            ...
            int size_needed = WideCharToMultiByte(CP_UTF8, 0, &filename[0], (int)wcslen(filename),
                                                  NULL, 0, NULL, NULL);
            std::string path(size_needed, 0);
            WideCharToMultiByte(CP_UTF8, 0, &filename[0], (int)wcslen(filename), &path[0],
    ...
    void GuiManager::load_archive(const fs::path &pak_path)
    {
        ...
        try
        {
            ...
            if (!parser || !parser->parse())
            {
                ...
                MessageBoxW(main_window, L"Failed to parse archive!", L"Error",
                            MB_OK | MB_ICONERROR);
                parser = nullptr;
                is_loading = false;
                error_msg.clear();
            ...
            {
                std::cerr << "[WARNING] Archive validation failed with "
                          << validation.invalid_entries << " invalid entries" << std::endl;
            }

            Logger::instance().success(std::string("Archive loaded successfully: ") +
                                       pak_path.string());
            Logger::instance().info(std::string("Format: ") + parser->get_format_info());
            Logger::instance().info(std::string("Files: ") +
                                    std::to_string(parser->get_file_count()));

            char status_buffer[256];
            snprintf(status_buffer, sizeof(status_buffer), "Archive loaded: %u files",
                     parser->get_file_count());
    ...
    void GuiManager::populate_tree_view()
    {
        ...
        try
        {
            ...
            DEBUG_COUT("[DEBUG] TreeView: Subdirectories count: " << root->subdirectories.size()
                                                                  << std::endl);
    ...
    void GuiManager::preview_file(const std::shared_ptr<FileEntry> &file)
    {
        ...
        static constexpr const char *supported_exts[] = {
            "txt", "cfg", "ini", "md", "log", "conf", "config", "properties", "xml", "json"};
        ...
        for (const char *supported : supported_exts)
        {
            ...
            {
                char ext_lower =
                    static_cast<char>(std::tolower(static_cast<unsigned char>(*ext_check)));
                if (ext_lower != *sup_check)
                    break;
                ++ext_check;
                ++sup_check;
            }
        ...
        try
        {
            ...
            int len = MultiByteToWideChar(CP_UTF8, 0, content.data(),
                                          static_cast<int>(content.size()), NULL, 0);
            if (len > 0)
            {
                wide_content.resize(len + 1, 0);
                int converted =
                    MultiByteToWideChar(CP_UTF8, 0, content.data(),
                                        static_cast<int>(content.size()), wide_content.data(), len);
                if (converted > 0 && converted <= len)
                {
                    ...
                    if (has_data)
                    {
                        ...
                        DEBUG_COUT("[DEBUG] Converted as UTF-8: " << converted << " wide chars"
                                                                  << std::endl);
                    }
                    else
                    {
                        std::cerr << "[WARNING] UTF-8 conversion produced only null bytes"
                                  << std::endl;
                    }
                }
                else
                {
                    ...
                    std::cerr << "[WARNING] UTF-8 conversion failed, converted=" << converted
            ...
            {
                int len_default = MultiByteToWideChar(CP_ACP, 0, content.data(),
                                                      static_cast<int>(content.size()), NULL, 0);
                if (len_default > 0)
                {
                    wide_content.resize(len_default + 1, 0);
                    int converted = MultiByteToWideChar(CP_ACP, 0, content.data(),
                                                        static_cast<int>(content.size()),
                                                        wide_content.data(), len_default);
                    if (converted > 0 && converted <= len_default)
                    {
                        ...
                        {
                            conversion_success = true;
                            DEBUG_COUT("[DEBUG] Converted as system default code page: "
                                       << converted << " wide chars" << std::endl);
                        }
                        else
                        {
                            std::cerr
                                << "[WARNING] System code page conversion produced only null bytes"
                                << std::endl;
                        }
                    }
                }
            ...
            if (conversion_success && !wide_content.empty())
            {
                ...
                {
                    DEBUG_COUT("[DEBUG] Previewed file: " << file->path << " (" << file->size
                                                          << " bytes, " << (wide_content.size() - 1)
                                                          << " wide chars)" << std::endl);
                }
                else
                {
                    ...
                    std::cerr << "[ERROR] SetWindowTextW failed or text not set, result=" << result
                              << ", text_was_set=" << text_was_set << ", error: " << error
                              << std::endl;
                    DEBUG_CERR("[DEBUG] First 50 chars of content: ");
#ifdef _DEBUG
                    size_t max_chars =
                        (wide_content.size() > 1)
                            ? ((wide_content.size() - 1 < 50) ? (wide_content.size() - 1) : 50)
                            : 0;
            ...
            else
            {
                ...
                if (error != 0)
                {
                    std::cerr << "[ERROR] Failed to convert file content to wide string, error: "
                              << error << std::endl;
                }
                else
                {
                    std::cerr << "[ERROR] Failed to convert file content to wide string"
                              << std::endl;
                }
    ...
    GuiManager::find_file_in_tree(const std::shared_ptr<DirectoryEntry> &dir,
    ...
    void GuiManager::check_for_updates()
    {
        ...
        HINTERNET hSession = WinHttpOpen(L"unPAKer/1.0", WINHTTP_ACCESS_TYPE_DEFAULT_PROXY,
        ...
        if (!hSession)
        {
            ...
            MessageBoxW(main_window, error_msg.c_str(), L"Update Check Failed",
                        MB_OK | MB_ICONERROR);
            error_msg.clear();
            error_msg.shrink_to_fit();
        ...
        HINTERNET hRequest =
            WinHttpOpenRequest(hConnect, L"GET", L"/repos/mxtherfxcker/unPAKer/releases", nullptr,
        ...
        BOOL bResult = WinHttpSendRequest(hRequest, headers, static_cast<DWORD>(-1),
        ...
        if (statusCode != 200)
        {
            ...
            do
            {
                ...
                if (!WinHttpReadData(hRequest, &errorBuffer[oldSize], bytesAvailable, &bytesRead))
                    break;
            ...
            if (!errorBuffer.empty())
            {
                ...
                DEBUG_COUT("[DEBUG] Error response body: " << errorResponse.substr(0, 500)
                                                           << std::endl);
            }
            errorBuffer.clear();
        ...
        if (buffer.empty())
        {
            MessageBoxW(main_window, L"Failed to read response from GitHub API.",
                        L"Update Check Failed", MB_OK | MB_ICONERROR);
            set_status_text("Update check failed");
            buffer.clear();
        ...
        std::string debugPreview =
            jsonResponse.substr(0, (jsonResponse.length() < 500) ? jsonResponse.length() : 500);
        ...
        if (!isArray)
        {
            MessageBoxW(main_window,
                        L"Unexpected response format from GitHub API (expected array).",
                        L"Update Check Failed", MB_OK | MB_ICONERROR);
            set_status_text("Update check failed");
            return;
        }
        ...
        while (pos < jsonResponse.length())
        {
            ...
            if (std::regex_search(releaseObj, match, tagRegex) && match.size() > 1)
            {
                ...
                {
                    latestVersion = tagName;
                    DEBUG_COUT("[DEBUG] Found prerelease (storing as fallback): " << latestVersion
                                                                                  << std::endl);
                }
            }
        ...
        if (!found)
        {
            std::wstring error_msg =
                L"Failed to parse version information from GitHub API response.\n\n";
            error_msg += L"Response preview (first 200 chars):\n";
            std::string preview =
                jsonResponse.substr(0, (jsonResponse.length() < 200) ? jsonResponse.length() : 200);
                    ...
                    else
                    {
                        ...
                        else
                        {
                            auto getSuffixPriority = [](const std::string &suffix) -> int
                            {
                                ...
                                if (suffix.find("rc") == 0)
                                    return 4;
                                if (suffix.find("beta") == 0)
                                    return 3;
                                if (suffix.find("alpha") == 0)
                                    return 2;
                                if (suffix.find("dev") == 0)
                                    return 1;
                                return 0;
                            };
