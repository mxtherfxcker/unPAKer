...
namespace unpaker
{
                                                    ...
                                                    uint64_t archive_size)
    {
        ...
        Logger::instance().info(std::string("Validating archive with ") +
                                std::to_string(result.total_files) + " files...");

        std::vector<std::string> duplicates;
        result.duplicate_files = checkDuplicates(all_files, duplicates);
        if (result.duplicate_files > 0)
        {
            result.warnings.push_back("Found " + std::to_string(result.duplicate_files) +
        ...
        {
            Logger::instance().warning(std::string("Archive validation found ") +
                                       std::to_string(result.error_messages.size()) + " errors:");
        ...
        {
            Logger::instance().info(std::string("Found ") + std::to_string(result.warnings.size()) +
                                    " warnings:");
