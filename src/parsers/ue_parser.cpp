...
namespace unpaker::parsers
{
    ...
    bool UEParser::parse(const fs::path &archive_path, std::shared_ptr<DirectoryEntry> &root,
                         uint32_t &file_count)
    {
        FILE *file;
        ...
        {
            Logger::instance().warning(std::string("UE: Suspicious entry count: ") +
                                       std::to_string(entry_count) +
                                       std::string(", adjusting to 256"));
            entry_count = 256;
        }
        ...
        Logger::instance().info(std::string("UE: Successfully parsed ") +
                                std::to_string(file_count_local) + std::string(" file entries"));
                                ...
                                std::vector<uint8_t> &data) const
    {
        ...
        try
        {
            ...
            if (!ifs.is_open())
            {
                std::cerr << "[ERROR] UE: Failed to open archive file: " << archive_path.string()
                          << std::endl;
                return false;
            }
            ...
            if (!ifs.good())
            {
                std::cerr << "[ERROR] UE: Failed to seek to file offset: " << file->offset
                          << std::endl;
                return false;
            }
